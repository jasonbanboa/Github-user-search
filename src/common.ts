...
const $main = document.querySelector('.card > main') as HTMLDivElement;
...
const util = {
  ...
  resetForm({ name , login }: UserData): void {
    $form.reset();
    $input.placeholder = name ?? login;
  },
  handleError(query: string) {
    $main.innerHTML = `<div class="low-contrast">Error user ${query} Not Found...</div>`;
  }
}
...
const handler = {
  async formSubmit(e: Event) {
    e.preventDefault();
    const userData: UserData = await fetchUserData($input.value); 
    if (!userData.login) {
      return util.handleError($input.value);
    }
    renderCard(userData);
    util.resetForm(userData);
  },
  changeTheme(): void {
    const $body = document.querySelector('body') as HTMLBodyElement;
    $body.className = $body.className === 'light-mode' ? 'dark-mode' : 'light-mode';
    $theme.innerHTML = $body.className === 'light-mode' ? `
        DARK
...
async function fetchUserData(username: string): Promise<UserData> {
  const { 
    avatar_url, name, login, created_at, bio, public_repos,followers,
    following, location, twitter_username, blog, company,
...
window.onload = (): void => {
  main();
  $form?.addEventListener('submit', handler.formSubmit);
  $theme?.addEventListener('click', handler.changeTheme);
}
